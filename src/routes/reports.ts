...
router.get('/visits/excel', authenticateJWT, requireRole(['ADMIN']), async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/visits/pdf', authenticateJWT, requireRole(['ADMIN']), async (req: AuthRequest, res: Response) => {
  try {
    const visits = await prisma.visit.findMany({
      include: {
        customer: true,
    ...
    // Title / Header
    doc.fillColor('#0F172A').fontSize(24).text('INTEGRA TRADE', { align: 'center', bold: true } as any);
    ...
    for (let i = 0; i < visits.length; i++) {
      ...
      doc.fillColor('#0F172A').fontSize(14).text(`Visita: ${v.customer.name} (${v.customer.code})`, { bold: true } as any);
      ...
      if (v.exhibitions) {
        doc.moveDown(0.5);
        doc.fillColor('#0F172A').fontSize(10).text('Checklist de Exhibición:', { bold: true } as any);
      ...
      if (v.photos && v.photos.length > 0) {
        doc.moveDown(1);
        doc.fillColor('#0F172A').fontSize(11).text('Evidencia Fotográfica:', { bold: true } as any);
        doc.moveDown(0.5);
      ...
      if (v.signatureUrl) {
        const sigPath = path.join(__dirname, '../../', v.signatureUrl);
        if (fs.existsSync(sigPath)) {
          if (doc.y > 650) {
            doc.addPage();
          }
          doc.moveDown(1);
          doc.fillColor('#0F172A').fontSize(10).text('Firma Digital del Cliente:', { bold: true } as any);
