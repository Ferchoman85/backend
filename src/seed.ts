...
async function main() {
  ...
  await prisma.merchandiser.deleteMany({});
  ...
  const mercPassword = await bcrypt.hash('merc123', 10);
  const mercUser = await prisma.user.create({
    data: {
  ...
  const merchandiser = await prisma.merchandiser.create({
    data: {
      userId: mercUser.id,
